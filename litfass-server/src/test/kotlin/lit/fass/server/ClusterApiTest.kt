...
internal class ClusterApiTest : TestcontainerClusterSupport() {
    ...
    fun `collections {collection} GET endpoint`() {
        ...
        "${baseUrl2()}/collections/bar?foo=bar&param1=foo&param1=bar&param2=true"
            .httpGet()
            .response()
            .apply {
                val response = second
    ...
    fun `collections {collection} id GET endpoint`() {
        ...
        assertThat(aggregatedLogs()).contains("Getting collection data for bar with id 1 for user admin")
    }

    @Test
    @Order(3)
    fun `collections {collection} id GET endpoint for second entry`() {
        "${baseUrl1()}/collections/bar/2"
            .httpGet()
            .authentication().basic("admin", "admin")
            .responseObject<Map<String, Any?>>(jacksonDeserializerOf())
            .apply {
        ...
        assertThat(aggregatedLogs()).contains("Getting collection data for bar with id 2 for user admin")
    }
    ...
    fun `collections {collection} POST endpoint`() {
        ...
        "${baseUrl3()}/collections/foo?param1=foo&param1=bar&param2=true"
            .httpPost()
    ...
    fun `collections {collection} POST endpoint removes collection item`() {
        "${baseUrl3()}/collections/foo?action=delete"
            .httpPost()
        ...
        assertThat(aggregatedLogs()).contains("Removed collection foo")
